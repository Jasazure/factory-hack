...
{
    // Canonical fault → required technician skills mapping
    ...
    // Canonical fault → required spare part numbers mapping
    ...
    // Returns required skills for a fault type.
    // Falls back to ["general_maintenance"] for unknown faults.
    public IReadOnlyList<string> GetRequiredSkills(string faultType) =>
        FaultToSkills.TryGetValue(faultType, out var skills)
            ? skills
            : ["general_maintenance"];

    // Returns required part numbers for a fault type.
    // Falls back to empty list for unknown faults.
